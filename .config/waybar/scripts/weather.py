...
def main():
    ...
    if not api_key:
        print(
            json.dumps(
                {
                    "text": "󰖐 no key",
                    "tooltip": "Set API_KEY in ~/.config/waybar/weather.conf",
                }
            )
        )
        return
    ...
    try:
        ...
        tooltip = (
            ...
            f"{sep}\n"
            f"<span color='#ffcc66'>🤔  Ощущается как</span>   <span weight='bold'>{feels_like}{deg_sym}</span>\n"
            f"<span color='#88ccff'>⬇️  Мин</span>           <span weight='bold'>{temp_min}{deg_sym}</span>\n"
            f"<span color='#ff8888'>⬆️  Макс</span>           <span weight='bold'>{temp_max}{deg_sym}</span>\n"
            f"{sep}\n"
            f"<span color='#88ddff'>💧  Влажность</span>     <span weight='bold'>{humidity}%</span>\n"
            f"<span color='#aaffaa'>💨  Ветер</span>          <span weight='bold'>{wind_speed} km/h</span>\n"
            f"<span color='#ccaaff'>🔵  Давление</span>     <span weight='bold'>{pressure} hPa</span>"
        )
